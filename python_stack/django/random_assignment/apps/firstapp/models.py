...
class UserManager(models.Manager):
    ...
    def isValidRegistration(self, user_info):
        ...
        if len(error_messages) == 0:
            ...
            User.objects.create(first_name = user_info['first_name'], last_name = user_info['last_name'], date_of_birth = user_info['date_of_birth'], email = user_info['email'], password = hashed)
        return error_messages

class User(models.Model):
    first_name = models.CharField(max_length=100)
    last_name = models.CharField(max_length=100)
    date_of_birth = models.DateField(auto_now=False, auto_now_add=False, default=10/10/2001)
