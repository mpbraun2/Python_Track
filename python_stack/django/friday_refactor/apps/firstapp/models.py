...
class AppointmentManager(models.Model):
    def isValidAppointment (self, appointment_info):
        error_messages=[]
        if not appointment_info['task'].isalpha():
            error_messages.append('Task name contains non-alpha character(s)')
        if len(appointment_info['task']) < 2:
            error_messages.append('Task name is not long enough.')
        if len(error_messages) == 0:
            Appointment.objects.create(task = appointment_info['task'], time = appointment_info['time'], status = appointment_info['status'])
        return error_messages
...
class User(models.Model):
    ...
    dob = models.DateField(max_length=8, default='2000-10-10')
